...
export async function registerRoutes(
  ...
  app.post("/api/claim-daily-bonus", requireAuth, async (req, res) => {
    try {
      ...
      // Chaque pointage reçoit un montant aléatoire compris entre 0,10 et 0,40 USDT.
      // Le tirage reste côté serveur afin que le montant ne puisse pas être choisi par le client.
      ...
      await storage.createTransaction({
        ...
        description: `Bonus quotidien — pointage aléatoire de ${bonusAmount.toFixed(2)} USDT`,
      });

      res.json({
        success: true,
        amount: bonusAmount.toFixed(2),
        message: `Pointage validé : +${bonusAmount.toFixed(2)} USDT ajouté à votre solde`,
      });
    } catch (error: any) {
      res.status(500).json({ message: error.message });
    }
  ...
  app.get("/api/daily-bonus-status", requireAuth, async (req, res) => {
    try {
      ...
      const bonusTransactions = allTransactions.filter(
        (t: any) =>
          t.type === "bonus" &&
          (t.description === "Bonus quotidien" ||
            t.description?.startsWith("Bonus quotidien — pointage aléatoire de ")),
      );
