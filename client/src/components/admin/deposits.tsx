...
export default function AdminDeposits() {
  ...
  const reviewCount = allDeposits?.filter(d => d.status === "review").length || 0;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 overflow-x-auto">
        {(["pending", "processing", "review", "approved", "rejected", "all"] as const).map((status) => (
          <Button
            key={status}
            size="sm"
          ...
          >
            {status === "all" ? t.bankerAll : status === "pending" ? `${t.statusPending}${pendingCount > 0 ? ` (${pendingCount})` : ""}` : status === "processing" ? `${t.statusProcessing}${processingCount > 0 ? ` (${processingCount})` : ""}` : status === "review" ? `À examiner${reviewCount > 0 ? ` (${reviewCount})` : ""}` : status === "approved" ? t.statusApproved : t.statusRejected}
          </Button>
        ))}
      </div>
              ...
              <Card key={deposit.id} className={deposit.status === "pending" ? "border-yellow-400/50" : deposit.status === "processing" ? "border-blue-400/50" : deposit.status === "review" ? "border-amber-500/70" : ""}>
                <CardContent className="p-4 space-y-3">
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <Badge
                      variant={deposit.status === "pending" ? "secondary" : deposit.status === "approved" ? "default" : deposit.status === "processing" || deposit.status === "review" ? "outline" : "destructive"}
                      className={deposit.status === "processing" ? "bg-blue-600 text-white border-blue-600" : deposit.status === "review" ? "bg-amber-100 text-amber-900 border-amber-400" : ""}
                    >
                      {deposit.status === "pending" ? t.statusPending : deposit.status === "processing" ? t.statusProcessing : deposit.status === "review" ? "À examiner" : deposit.status === "approved" ? t.statusApproved : t.statusRejected}
                    </Badge>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm bg-secondary/50 rounded-xl p-3">
                    ...
                    {isNowPayments && (
                      <div className="col-span-2 grid grid-cols-2 gap-2 rounded-lg border border-blue-200 bg-blue-50/60 p-2 text-xs dark:border-blue-900 dark:bg-blue-950/20">
                        <div>
                          <p className="text-muted-foreground">Attendu</p>
                          <p className="font-medium">{(deposit as any).nowPaymentsExpectedAmount || "—"} {(deposit as any).nowPaymentsExpectedCurrency?.toUpperCase() || ""}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground">Reçu</p>
                          <p className="font-medium">{(deposit as any).nowPaymentsActuallyPaid || "—"} {(deposit as any).channelName || ""}</p>
                        </div>
                        <div className="col-span-2">
                          <p className="text-muted-foreground">Statut NOWPayments</p>
                          <p className="font-medium">{(deposit as any).nowPaymentsStatus || "En attente"}</p>
                        </div>
                        {(deposit as any).nowPaymentsError && (
                          <p className="col-span-2 text-amber-700 dark:text-amber-300">{(deposit as any).nowPaymentsError}</p>
                        )}
                      </div>
                    )}
                  </div>
                  ...
                  {/* Actions */}
                  {(deposit.status === "pending" || deposit.status === "processing" || deposit.status === "review") && (
                    <div className="flex gap-2">
                      {!isNowPayments && (
                        <Button
                          size="sm"
