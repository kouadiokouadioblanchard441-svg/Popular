...
export default function AdminWithdrawals() {
  ...
  const [statusFilter, setStatusFilter] = useState<"all" | "pending" | "pending_2fa" | "processing" | "reconciling" | "approved" | "rejected" | "failed">("pending");
  ...
  const toggleMode = async () => {
    const newMode = isManual ? "semi_auto" : "manual";
  ...
  const startNowPaymentsMutation = useMutation({
    mutationFn: async (id: number) => {
      const response = await apiRequest("POST", `/api/admin/withdrawals/${id}/start-nowpayments`, {});
      const data = await response.json();
      if (!response.ok) throw new Error(data.message || "Impossible de lancer NOWPayments");
      return data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/withdrawals"] });
      toast({
        title: "Payout NOWPayments créé",
        description: "Saisissez maintenant le code 2FA pour autoriser l'envoi.",
      });
    },
    onError: (error: any) => {
      toast({ title: error.message || t.errorOccurred, variant: "destructive" });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 overflow-x-auto">
        {(["all", "pending", "pending_2fa", "processing", "reconciling", "approved", "rejected", "failed"] as const).map((status) => (
          <Button
            key={status}
            size="sm"
              ...
              <CardContent className="p-4 space-y-3">
                <div className="flex items-start justify-between">
                  ...
                  <Badge variant={
                    ...
                    withdrawal.status === "processing" || withdrawal.status === "pending_2fa" || withdrawal.status === "reconciling" ? "outline" : "destructive"
                  }>
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  ...
                  {withdrawal.nowPaymentsPayoutId && (
                   ...
                   {withdrawal.nowPaymentsError && (
                     <div className="col-span-2 rounded-md bg-destructive/10 px-2 py-1 text-xs text-destructive">
                       NOWPayments : {withdrawal.nowPaymentsError}
                     </div>
                   )}
                   {withdrawal.nowPaymentsExternalId && !withdrawal.nowPaymentsPayoutId && (
                     <div className="col-span-2 text-xs text-muted-foreground">
                       Référence externe NOWPayments : {withdrawal.nowPaymentsExternalId}
                     </div>
                   )}
                  <div className="col-span-2">
                  ...
                  <div className="flex gap-2">
                    {isManual ? (
                      ...
                      <Button
                        ...
                        onClick={() => startNowPaymentsMutation.mutate(withdrawal.id)}
                        disabled={startNowPaymentsMutation.isPending}
                        data-testid={`button-start-nowpayments-${withdrawal.id}`}
                      >
                        {startNowPaymentsMutation.isPending
                    ...
                    <Button
                      ...
                      disabled={processMutation.isPending || startNowPaymentsMutation.isPending}
